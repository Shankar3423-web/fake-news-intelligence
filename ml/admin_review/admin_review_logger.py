...
def setup_logger(logs_dir: str = "ml/admin_review/logs") -> logging.Logger:
    """
    Sets up and configures the logger for the admin review pipeline.
    Creates a timestamped log file in the logs directory and configures logging.
    """
    os.makedirs(logs_dir, exist_ok=True)

    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = os.path.join(logs_dir, f"admin_review_pipeline_{timestamp}.log")
    static_log_file = os.path.join(logs_dir, "admin_review_pipeline.log")
    ...
    # Static file handler (overwrite/append to admin_review_pipeline.log)
    ...
    logger.info(f"Initialized admin review pipeline logger. Log file: {log_file}")
