...
class DatasetLoader:
    ...
    - Extracts testing split deterministically to guarantee alignment with Phase 6.
    """
    ...
    def load_and_validate(self) -> Tuple[pd.DataFrame, List[str]]:
        ...
        if missing_features:
            raise ValueError(f"Dataset is missing {len(missing_features)} expected features. Missing first 10: {missing_features[:10]}...")
    ...
    def load_test_split(self, config: EvaluationConfig) -> Tuple[pd.DataFrame, pd.Series]:
        """
        Loads the dataset, validates it, and performs train/test split to extract
        the exact test features and label Series.
        """
        df, selected_features = self.load_and_validate()
        
        logger.info("Extracting testing split deterministically...")
        ...
        logger.info(f"Testing split successfully loaded. Features shape: {X_test.shape}, Labels shape: {y_test.shape}")
